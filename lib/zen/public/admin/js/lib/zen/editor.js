 ...
 *
 * Once you have your element in place you can create a new instance by doing 
 * the following:
 *
 *     var editor = new Zen.Editor($('markdown_editor'));
 *
 * This will create a new editor instance using HTML as the markup. Because 
 * we've set an ID we can retrieve this instance as following:
 *
 *     Zen.Editor.instances.markdown_editor
 *
 * This means that it's not required to actually store the instance in a 
 * variable unless you're not using an ID.
 *
 * The first parameter of the Zen.Editor class can either be a string or an 
 * element. If it's a string it's assumed that it's a valid CSS selector and 
 * the first element found will be used. If it's an element it will be used 
 * directly without any further processing. A few examples can be seen below.
 *
 *     new Zen.Editor('#markdown_editor'); // => markdown_editor will be used
 *     new Zen.Editor($$('textarea.some_editor')); // => First item will be used
 *
 * The second parameter is an object with custom options. Currently there are 
 * only two options available:
 *
 * * height: sets a fixed height for the textarea.
 * * width: does the same as "height" but for the width.
 *
 * The third (and last) parameter is an array of custom buttons to add to the 
 * editor on top of the default buttons. This parameter should be an array of 
 * objects where each object has the following three keys:
 *
 * * name: a unique name for the button, also set as the class of the <li> 
 *   element.
 * * label: a label to display in the button, also used for the value of the 
 *   title attribute.
 * * onClick: function that's executed whenever the button is clicked. This 
 *   function can take a single parameter which is set to the instance of the 
 *   editor to which the button belongs.
 *
 ...
 * @return {object} An instance of the correct driver or an error class in case 
